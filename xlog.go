...
func colorForLevel(level LogLevel) string {
	...
	default:
		colorInt = colorDefault
	}
	
	colorStr := fmt.Sprintf("\033[%dm",int(colorInt))
	
	return colorStr
}

func prefixForLevel(level LogLevel) string {
	colorStr := ""
...
func formatStr(level LogLevel, str string) string {
	buf := &bytes.Buffer{}
	buf.Write([]byte(colorForLevel(level)))
	buf.Write([]byte(prefixForLevel(level)))
	buf.Write([]byte(str))
...
func doLogging(logLevel LogLevel, fileName string, maxBytes, backupCount int) {
	...
	if fileName != "" {
		...
		if err != nil {
			log.Fatal("hlog: unable to create RotatingFileHandler: ", err)
		}
	...
	Logger = hlog{
		Debug:   log.New(debugHandle, "", DefaultFlags),
...
func Debug(format string, a ...interface{}) {
	Logger.Debug.Output(2, formatStr(LevelDebug,fmt.Sprintf(format, a...)))
}
...
func Info(format string, a ...interface{}) {
	Logger.Info.Output(2, formatStr(LevelInfo,fmt.Sprintf(format, a...)))
}
...
func Warning(format string, a ...interface{}) {
	Logger.Warning.Output(2, formatStr(LevelWarn,fmt.Sprintf(format, a...)))
}
...
func Error(format string, a ...interface{}) {
	Logger.Error.Output(2, formatStr(LevelError,fmt.Sprintf(format, a...)))
}
